...
calculate_own_site_simil_score <- function(this_timeseries_wide) {
  ...
  dist_object_label_sites <- data.frame("subject_id" = dist_object_labels) %>%
    left_join(y=select(subjects, subject_id, site), by="subject_id") %>%
    pull(site)
  
  # Only process sites with more than one subject per time series
  sites_to_process <- subjects %>%
    filter(site %in% dist_object_label_sites) %>%
    filter(subject_id %in% dist_object_labels) %>%
    group_by(site) %>%
    summarise(subj_count = n_distinct(subject_id)) %>%
    filter(subj_count > 1) %>%
    pull(site)
