...
process_a_study <- function(subjects, parameters, data, custom_timeseries, timeseries_features_to_calculate,
  ...
  parameters <- parameters %>%
    mutate(time_point_count_min = ifelse(is.na(time_point_count_min), default_minimum_timepoints_per_series, time_point_count_min),
           subject_count_min = ifelse(is.na(subject_count_min), default_minimum_subjects_per_series, subject_count_min),
           max_share_missing = ifelse(is.na(max_share_missing), default_max_share_missing_timepoints_per_series, max_share_missing),
           generate_change_from_baseline = ifelse(is.na(generate_change_from_baseline), default_generate_change_from_baseline, generate_change_from_baseline)
    )
  ...
  timepoint_rank_to_name_mapping <- data %>%
    mutate(names = paste0(timepoint_1_name, "_", timepoint_2_name)) %>%
    distinct(parameter_id, timepoint_rank, names)
  ...
  timepoints_and_subjects <- data %>%
    ...
    mutate(has_baseline_value = ifelse(!is.na(baseline) & baseline != '', "Yes", "No")) %>%
    arrange(parameter_id, subject_id, timepoint_rank, has_baseline_value) %>%
    distinct(parameter_id, subject_id, timepoint_rank, has_baseline_value)
  ...
  if(autogenerate_timeseries) {
    ...
    timeseries_per_param <- timepoints_and_subjects %>%
      ...
      nest() %>%
      left_join(y=select(parameters, parameter_id, time_point_count_min, subject_count_min, max_share_missing, generate_change_from_baseline), by="parameter_id") %>%
      rename(dataset = data) %>%
      mutate(baseline = ifelse(generate_change_from_baseline == TRUE, list(c("original", "cfb")), "original")) %>%
      unnest(baseline) %>%
      rowwise() %>%
      mutate(output = list(pick_timepoint_combos(dataset, time_point_count_min, subject_count_min, max_share_missing, baseline))) %>%
      unnest(output) %>%
      ungroup() %>%
      mutate(timeseries_id = paste0('ts_', row_number(), '_autogen')) %>%
      mutate(timeseries_id = paste0(timeseries_id, '_', baseline)) %>%
    ...
    if( nrow(custom_timeseries) > 0 ) {
      ...
      timeseries_per_param <- timeseries_per_param %>%
        anti_join(y=select(custom_timeseries, parameter_id, timepoint_combo), by = NULL) # Remove any autogenerated timeseries that are in the list of custom timeseries
  ...
  if( nrow(custom_timeseries) > 0 ) {
    ...
    timeseries_per_param_custom <- custom_timeseries %>%
      left_join(y=select(parameters, parameter_id, subject_count_min, max_share_missing, generate_change_from_baseline), by="parameter_id") %>%
      rowwise() %>%
      mutate(baseline = ifelse(generate_change_from_baseline == TRUE, list(c("original", "cfb")), "original")) %>%
      unnest(baseline) %>%
      mutate(timepoint_combo_subjects = pick_subjects_for_custom_timeseries(timepoints_and_subjects, timepoint_combo, max_share_missing, parameter_id, baseline)) %>%
      filter(str_count(timepoint_combo_subjects, ";") + 1 >= subject_count_min) %>% # Only pass time series with enough subjects.
      mutate(timeseries_id = paste0(timeseries_id, '_', baseline)) %>%
  ...
  results <- timeseries_per_param %>%
    filter(baseline == "original" | timepoint_count > 1) %>% # The latter condition requires that change-from-baseline series must have more than one time point.
    rowwise() %>%
    mutate(timeseries_wide = list(generate_wide_timeseries_table(parameter_id, timepoint_combo, timepoint_combo_subjects, baseline))) %>%
    filter(nrow(timeseries_wide) > 0) %>%
    mutate(principal_components = list(calculate_principal_components(timeseries_wide))) %>%
    mutate(ts_features = list(calculate_ts_features(timeseries_wide, baseline, timeseries_features_to_calculate))) %>%
    ungroup()

  # Parse the timepoint combination string into a more readable format.
  readable_timepoint_combo_names <- results %>%
    distinct(timepoint_combo, parameter_id) %>%
    rowwise() %>%
    mutate(timepoint_combo_readable = parse_readable_timeseries_combo_string(timepoint_combo, parameter_id, timepoint_rank_to_name_mapping))
  ...
  tso_timeseries <- results %>%
    ...
    select(timeseries_id, parameter_id, baseline, timepoint_combo, timepoint_combo_readable, timepoint_count)

  # Parse time series features into a proper format.
  tso_features <- results %>%
    select(timeseries_id, ts_features) %>%
    unnest(ts_features) %>%
    left_join(y=subjects, by="subject_id") %>%
    rename(feature_value = value) %>%
    relocate(timeseries_id, subject_id, feature, feature_value)

  # Parse the PCA coordinates into its own result table.
  tso_pca_coordinates <- results %>%
    select(timeseries_id, principal_components) %>%
    unnest(principal_components) %>%
    left_join(y=subjects, by="subject_id") %>%
    rename(pc1 = PC1, pc2 = PC2) %>%
    select(timeseries_id, subject_id, pc1, pc2)
  ...
  tso_site_scores <- results %>%
    ...
    left_join(y=select(parameters, parameter_id, subject_count_min), by="parameter_id") %>% # Look up minimum subjects expected per parameter.
    group_by(timeseries_id, feature) %>%
    filter( n_distinct(site) >= 2 & n() >= subject_count_min) %>%
    nest()
  ...
  if(nrow(tso_site_scores) >= 1) {
    ...
    tso_site_scores <- tso_site_scores %>%
      rowwise() %>%
      mutate(site_p_values = list(calculate_site_bias_ts_features(feature, data))) %>%
      select(-data) %>%
      unnest(site_p_values) %>%
      mutate(pvalue_kstest = as.numeric(pvalue_kstest), kstest_statistic = as.numeric(kstest_statistic)) %>%
      ungroup() %>%
      mutate(fdr_adjusted_pvalue_ks = p.adjust(pvalue_kstest, method = "fdr"),
             reference_group = "all") %>% # Correct for multiple testing
      mutate(pvalue_kstest_logp = -log10(pvalue_kstest), fdr_corrected_pvalue_logp = -log10(fdr_adjusted_pvalue_ks)) %>%
      mutate(pvalue_kstest_logp = if_else( is.infinite(pvalue_kstest_logp), 30, pvalue_kstest_logp),
             fdr_corrected_pvalue_logp = if_else( is.infinite(fdr_corrected_pvalue_logp), 30, fdr_corrected_pvalue_logp)) %>%
      select(timeseries_id, site, country, feature, pvalue_kstest_logp, kstest_statistic, fdr_corrected_pvalue_logp, reference_group, subj_count) %>%
      rename(subject_count = subj_count)
...
calculate_site_bias_ts_features <- function(this_feature, this_data) {
  ...
  nullhypo_ks <- case_when(
    ...
    this_feature == "unique_value_count" ~ "greater",
    TRUE ~ "two.sided"
  )

  site_metadata <- this_data %>%
    group_by(site) %>%
    summarise(country = first(country), subj_count = n_distinct(subject_id))

  # Iterate across sites
  for(this_site in unique(this_data$site)) {

    # Compare the feature values of the current site with the feature values of all the other sites.
    within_set_values <- filter(this_data, site == this_site)
    outside_set_values <- filter(this_data, site != this_site)

    # Kolmogorov-Smirnov test
    ks_results <- ks.test(x = within_set_values$value, y = outside_set_values$value, alternative = nullhypo_ks)
...
parse_readable_timeseries_combo_string <- function(timeseries_combo, this_parameter_id, this_timepoint_rank_to_name_mapping) {
  ...
  for(this_timepoint in timepoints) {
    ...
    timepoint_components <- this_timepoint_rank_to_name_mapping %>%
      filter(timepoint_rank == this_timepoint & parameter_id == this_parameter_id) %>%
      pull(names)
...
#'
#' @author Pekka Tiikkainen, \email{pekka.tiikkainen@@bayer.com}
calculate_own_site_simil_score <- function(this_timeseries_wide) {

  # For a time series, calculate its own site similarity score, i.e. rank all study subjects
  # by their similarity, and use ROC AUC to quantify whether subjects from the same site
  # are enriched in the beginning of the list.

  subj_keys <- this_timeseries_wide$subject_id

  this_timeseries_wide$subject_id <- NULL

  origvalues_distances <- as.data.frame( as.matrix(  dist(this_timeseries_wide)  ) )

  colnames(origvalues_distances) <- subj_keys
  origvalues_distances$subj_1 <- as.character( subj_keys )


  origvalues_distances_long <- origvalues_distances %>%
    pivot_longer(cols=-subj_1, names_to="subj_2", values_to="dist") %>%
    filter(subj_1 != subj_2) %>%
    left_join(y=select(subjects, subject_id, site), by=c("subj_1" = "subject_id")) %>%
    rename(subj_1_site = site) %>%
    left_join(y=select(subjects, subject_id, site), by=c("subj_2" = "subject_id")) %>%
    rename(subj_2_site = site) %>%
    mutate(same_site = ifelse(subj_1_site == subj_2_site, 1, 0))


  same_site_simil_metric <- origvalues_distances_long %>%
    group_by(subj_1) %>%
    filter(sum(same_site) > 0) %>% # Exclude subjects which are the only ones for the site
    summarise(own_site_simil_score = auroc(dist, same_site))
...
#'
#' @author Pekka Tiikkainen, \email{pekka.tiikkainen@@bayer.com}
calculate_ts_features <- function(this_timeseries_wide, this_baseline, this_timeseries_features_to_calculate) {

  own_site_similarity_scores <- calculate_own_site_simil_score(this_timeseries_wide)
  ...
  if(ncol(this_timeseries_wide) > 1 & this_baseline == "original") {
    ...
    simple_ts_features <- this_timeseries_wide %>%
      rowwise() %>%
      mutate(measurement_count = sum( !is.na(c_across(timepoint_col_names))) ) %>%
      mutate(average = mean(c_across(timepoint_col_names), na.rm=TRUE),
             sd = sd(c_across(timepoint_col_names), na.rm=TRUE),
             unique_value_count_relative = n_distinct(c_across(timepoint_col_names), na.rm=TRUE) / measurement_count,
             autocorr = ifelse(sd==0, NA, calculate_autocorrelation(c_across(timepoint_col_names))) # Only calculate auto-correlation if sd is non-zero
      ) %>%
      select(-c(all_of(timepoint_col_names), measurement_count)) %>%
      ungroup() %>%
      mutate(subject_id = subj_keys) %>%
      pivot_longer(cols=-subject_id, names_to="feature", values_to="value", values_drop_na = TRUE)


    own_site_similarity_scores <- own_site_similarity_scores %>%
      pivot_longer(cols=-subj_1, names_to="feature", values_to="value", values_drop_na = TRUE) %>%
      rename(subject_id = subj_1)

    ts_features <- bind_rows(simple_ts_features, own_site_similarity_scores)
    ...
    ts_features <- this_timeseries_wide %>%
      rowwise() %>%
      mutate(average = mean(c_across(timepoint_col_names), na.rm=TRUE)
      ) %>%
      select(-timepoint_col_names) %>%
      ungroup() %>%
      mutate(subject_id = subj_keys) %>%
      left_join(y=own_site_similarity_scores, by=c("subject_id"="subj_1")) %>%
      pivot_longer(cols=-subject_id, names_to="feature", values_to="value", values_drop_na = TRUE)
    ...
    ts_features <- this_timeseries_wide %>%
      rowwise() %>%
      mutate(average = mean(c_across(timepoint_col_names))
      ) %>%
      select(-timepoint_col_names) %>%
      ungroup() %>%
      mutate(subject_id = subj_keys) %>%
  ...
  ts_features <- ts_features %>%
    filter(feature %in% this_timeseries_features_to_calculate)
...
#'
#' @author Pekka Tiikkainen, \email{pekka.tiikkainen@@bayer.com}
generate_wide_timeseries_table <- function(this_parameter_id, this_timepoints, this_subjects, this_baseline) {
  ...
  measurements <- data %>%
    ungroup() %>%
    filter(timepoint_rank %in% this_timepoints & parameter_id == this_parameter_id & subject_id %in% this_subjects) %>%
  ...
  timeseries_wide <- measurements %>%
    pivot_wider(id_cols = subject_id, names_from = timepoint_rank, values_from = result, values_fn = mean)
...
pick_subjects_for_custom_timeseries <- function(this_timepoints_and_subjects, this_timepoints_string, this_max_share_missing, this_parameter_id, this_baseline) {
  ...
  timepoint_combo_subjects <- this_timepoints_and_subjects %>%
    filter(parameter_id == this_parameter_id) %>%
    select(subject_id, timepoint_rank) %>%
    filter(timepoint_rank %in% this_timepoints_string) %>%
    group_by(subject_id) %>%
    summarise(measurement_count = n()) %>%
    filter(measurement_count >= ceiling( (1 - this_max_share_missing) * timepoint_count)) %>%
    pull(subject_id)
  ...
  if(length(timepoint_ranks) >= this_time_point_count_min) {
    ...
    for(this_last_visit_index in length(timepoint_ranks):this_time_point_count_min) {
      ...
      timeseries_subjects <- dataset %>%
        filter(timepoint_rank %in% this_ts_timepoints) %>%
        group_by(subject_id) %>%
        summarise(measurement_count = n()) %>%
        filter(measurement_count >= ceiling( (1 - this_max_share_missing) * timepoint_count)) %>%
        pull(subject_id)
